...
def handle_input():
    ...
    if keys[pygame.K_a]:
        display_scroll[0] += -constants.PLAYER_SPEED
    if keys[pygame.K_d]:
        display_scroll[0] -= -constants.PLAYER_SPEED
    if keys[pygame.K_w]:
        display_scroll[1] -= constants.PLAYER_SPEED
    if keys[pygame.K_s]:
        display_scroll[1] += constants.PLAYER_SPEED
...
#Create Items:
coin = Item(400, 300, "assets/images/items/coin_f0.png", 100, "DEFAULT")
...
while run:
    ...
    coin.rect.x -= display_scroll[0]
    coin.rect.y -= display_scroll[1]
